...
print("\n" + "="*80)
print("  GEMMA 2 9B - PRODUCTION RUN (IMPROVED)".center(80))
print("  Full questions + combined data".center(80))
print("="*80 + "\n")
...
# Training
print("[STEP 5] PRODUCTION RUN - Training...")
print("="*80)
print(f"  Started: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
print(f"  Samples: {len(train_df)} training")
print(f"  Batch: 4 + 2 accumulation = effective 8")
print(f"  Epochs: 5 (full convergence)")
print(f"  Learning rate: 2e-4")
print(f"  Expected: ~90-120 minutes, loss 1-3 range")
print("="*80 + "\n")

from transformers import TrainingArguments, Trainer, DataCollatorForLanguageModeling

training_args = TrainingArguments(
    output_dir="gemma2_production_improved",
    num_train_epochs=5,
...
print("\n" + "="*80)
print("  PRODUCTION RUN COMPLETE!".center(80))
print("="*80)
print(f"  Finished: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
print(f"  Final loss: {trainer_stats.training_loss:.4f}")

model.save_pretrained("gemma2_production_improved")
tokenizer.save_pretrained("gemma2_production_improved")

print(f"  Model saved: gemma2_production_improved/\n")

print("="*80)
print("  READY FOR TESTING!".center(80))
print("="*80)
